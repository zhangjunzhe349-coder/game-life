...
`js/life.js` 把「一周一格」的一生画到 `#life-grid` 画布上（52 列 = 一年，行 = 岁）。

**列宽必须按容器宽度算，而不是按画布自己算** —— 这是旧版最难看的一处：

| | 旧（≤ v1.5.0） | 新（v1.6.0） |
|---|---|---|
| 宽度基准 | `canvas.clientWidth`（无 CSS 宽度时默认 **300px**） | `#life-grid-wrap` 的 `clientWidth` |
| 结果 | 只画出左边一小块，右侧一大片空白 | 整行铺满容器 |
| 跟随变化 | `window.resize` + `{ once: true }`（只响一次，且每次 render 叠加一个监听） | `ResizeObserver` + 宽度去重（`lastW`） |

两个实现要点：

1. **列宽取整 + 余数分摊**：先算 `base = floor(raw)`，再把除不尽的余数
   （`0 ~ 51` 个像素）分给**前 N 列**各 +1px。这样整行恰好铺满，且每列边缘都落在整数像素上
   （不用 `floor` 直接放弃余数，也不留半透明糊边）。
2. **`ctx.setTransform(dpr,0,0,dpr,0,0)` 而不是 `ctx.scale()`** —— 同一个 canvas 会被重画多次
   （容器尺寸变化时），`scale` 会累乘导致越画越大。
3. **去重防自激**：绘制会改画布高度 → 容器高度变化 → `ResizeObserver` 再次触发。
   用 `lastW` 记住上次的可用宽度，宽度没变就直接返回。`render()` 里把 `lastW` 重置为 `-1`，
   因为面板重建后 canvas 是新节点，必须重画。

几个关联约束：

- 生命面板初始是 `hidden`，隐藏时容器宽度为 0 → 绘制函数直接返回（等有宽度再画）。
  所以 `app.js` 的 `initTabs()` 里切到生命页时会显式调一次 `GL.renderLife()`。
- 宽屏下格子会跟着变大（1600px 窗口下约 22px/格，整块约 1850px 高）。
  若要压缩高度，在 `drawGrid()` 里给 `base` 加个上限（如 `Math.min(base, 14)`）并把整块居中即可。
...
- `verify-wiring.js`：补前者的盲区 —— 脚本顺序必须为
  ...
  **每个模块调用的 `GL.*` 都有提供者**、`sw.js` 的 `ASSETS` 完整且缓存版本 ≥ v8。
...
- `browser-check.js`（**最贴近真实的一层**）：内置静态服务器 + CDP 驱动真实浏览器，
  ...
  **并真点一遍新交互**（折叠、＋1 步进、−1 还原、折叠状态持久化、极性文案方向，
  ...
  输出各 tab 的 PNG + 编辑态局部图，并测量生命刻度是否铺满容器宽度。
